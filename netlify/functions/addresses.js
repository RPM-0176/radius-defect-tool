...
//
// Pulls from the SDA Project Pipeline board ONLY (5025662448) — this is the
// one board field inspections are logged against.
//
// To REMOVE a property from the dropdown once the house is done: just move
// its item into the "Fully Leased Properties" group on this board. Nothing
// to configure here — this endpoint excludes any group whose name contains
// "leased" or "completed", so it doesn't rely on a hardcoded group ID.
//
...
// Any group whose title contains one of these (case-insensitive) is treated
// as "done" and excluded — matches "Fully Leased Properties" without needing
// its exact group ID hardcoded.
const DONE_GROUP_NAME_PATTERN = /leased|completed/i;

// Non-property rows that sometimes sit in the board (a fresh template row, a
// duplicate-and-forgot-to-rename row) — filtered out by name so they never
// show up as a selectable address.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const addresses = items
      .filter(it => !it.group || !DONE_GROUP_NAME_PATTERN.test(it.group.title || ''))
