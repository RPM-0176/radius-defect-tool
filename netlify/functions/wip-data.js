...
exports.handler = async (event) => {
  ...
  async function getWipIndex() {
    const r = await ghGetRaw(WIP_INDEX_PATH);
    if (!r.exists) return [];
    return JSON.parse(Buffer.from(r.content, 'base64').toString('utf-8'));
  }
  ...
  if (event.httpMethod === 'GET' && qp.download) {
    ...
    try {
      ...
      if (!r.exists) return { statusCode: 404, body: JSON.stringify({ error: 'Could not fetch the in-progress data.' }) };
