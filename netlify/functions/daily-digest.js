...
exports.handler = async () => {
  ...
  let mondayByPerson = {};
  let crossCheckFlags = []; // { person, name, label, daysSince, source }
    ...
    try {
      ...
      for (const item of items) {
        ...
        for (const cc of CROSS_CHECK_STAGES) {
          ...
          crossCheckFlags.push({ person, name: item.name, label: cc.label, daysSince: daysSinceTrigger, source });
        }
  ...
  html += `<h3 style="color:#1B2430;">Cross-check: inspections that may be overdue</h3>`;
  if (mondayError) {
    html += `<p style="color:#999;">Skipped \u2014 couldn't load Monday.com today.</p>`;
  } else if (crossCheckFlags.length === 0) {
    html += `<p style="color:#3C7A5C;">Nothing flagged \u2014 every stage reached recently has a matching inspection in the checklist tool.</p>`;
  } else {
    html += `<ul>`;
    for (const f of crossCheckFlags) {
      html += `<li style="color:#A8461F;"><b>${f.name}</b> (${f.person}) \u2014 no ${f.label} inspection found in the checklist tool, ${f.daysSince} day${f.daysSince===1?'':'s'} since ${f.source}</li>`;
    }
    html += `</ul>`;
  }
